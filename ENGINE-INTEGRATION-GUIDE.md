...
persistence, circuit validation, and Qiskit Aer simulation.
...
(default `1024`, maximum `10000`). Simulation returns both measurement
...
If Qiskit is not installed or available, simulation-dependent results return
an explicit unavailable state and never claim to have simulated the circuit.
...
| `422` | Invalid circuit or request input | Render gate-level validation feedback |
